...
export async function getAggregateDashboard(params: AggregateParams): Promise<AggregateDashboard> {
  const snapshot = await computeOpenSnapshot(params.teamIds);
...
function assembleAggregate(
  ...
  return {
    ...
    totalTeams: params.teamIds.length,
...
async function computeOpenSnapshot(teamIds: string[]) {
  const threads = await loadOpenThreadsAcrossTeams(teamIds);
  const now = Date.now();
...
function recordImpactedOrg(map: Map<string, { name: string; count: number }>, thread: OpenThreadForBucketing): void {
  const existing = map.get(thread.teamId);
  if (existing) existing.count++;
  else map.set(thread.teamId, { name: thread.team.name, count: 1 });
}

function buildImpactedList(map: Map<string, { name: string; count: number }>): ImpactedOrg[] {
  const entries = Array.from(map.entries()).map(([id, v]) => ({ teamId: id, teamName: v.name, overdueCount: v.count }));
  return entries.sort((a, b) => b.overdueCount - a.overdueCount);
}

async function loadOpenThreadsAcrossTeams(teamIds: string[]) {
  return prisma.thread.findMany({
    ...
    select: {
      teamId: true,
      lastInboundAt: true,
      team: { select: { name: true, settings: { select: { slaMinutes: true } } } },
    },
  });
}
...
interface OpenThreadForBucketing {
  ...
  team: { name: string; settings: { slaMinutes: number } | null };
}

function classifyUrgency(thread: OpenThreadForBucketing, now: number): "overdue" | "at_risk" | "on_track" {
  const slaMin = thread.team.settings?.slaMinutes ?? DEFAULT_SLA_MINUTES;
